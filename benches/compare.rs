...
macro_rules! bench {
    ($name:expr, $body:expr) => {{
        ...
        println!("{:<30} {:>10}  {:>8.3?}  {:>10}", $name, result, dur, stringify!($body));
    }};
}
...
fn main() {
    ...
    bench!("emk_comb_gen(10,5)", {
        let mut cnt = 1;
        for _ in emk_comb_gen(10, 5) { cnt += 1; }
        cnt
    });
    ...
    bench!("sjt_gen(8)", {
        let mut cnt = 0;
        for _ in sjt_gen(8) { cnt += 1; }
        cnt
    });
    bench!("sjt_gen(7)", {
        let mut cnt = 0;
        for _ in sjt_gen(7) { cnt += 1; }
        cnt
    });
    bench!("sjt_gen(5)", {
        let mut cnt = 0;
        for _ in sjt_gen(5) { cnt += 1; }
        cnt
    });
    ...
    bench!("brgc_gen(12)", {
        let mut cnt = 0;
        for _ in brgc_gen(12) { cnt += 1; }
        cnt
    });
    bench!("brgc_gen(8)", {
        let mut cnt = 0;
        for _ in brgc_gen(8) { cnt += 1; }
        cnt
    });
    ...
    bench!("set_partition_gen(8,4)", {
        let mut cnt = 1;
        for _ in set_partition_gen(8, 4) { cnt += 1; }
        cnt
    });
    ...
    bench!("set_bipart_gen(15)", {
        let mut cnt = 1;
        for _ in set_bipart_gen(15) { cnt += 1; }
        cnt
    });
    bench!("set_bipart_gen(10)", {
        let mut cnt = 1;
        for _ in set_bipart_gen(10) { cnt += 1; }
        cnt
    });
